...
function Dashboard() {
  ...
  const getAllUsers = () => {
    UserService.getAllUsers().then(
      (response) => {
        ...
        const data2 = response.data.data.filter(e => e.role === '1');
        setDataUsers(data);
        setDataGuru(data2);
        // setting donut chart
        setDoughnutOptions({
  ...
  const getAllTugas = () => {
    TugasService.getAllTugas().then(
      (response) => {
        if(response.data.tugas.length) {
          setDataTugas(response.data.tugas);
          setDataSoal(response.data.tugas.reduce((a,b) => a.total_soal + b, 0));
        }
      },
      (error) => {
        console.log("Private page", error.response);
  ...
  return (
    <>
      ...
      <div className="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-4">
        <InfoCard title="Total Guru" value={dataGuru.length}>
          <RoundIcon
            icon={UserGroupIcon}
