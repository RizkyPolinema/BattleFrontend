...
import { ChevronLeftIcon, CurrencyDollarIcon } from '@heroicons/react/solid';
...
export default function Toko() {
    ...
    const updateCoin = () => {
        if (currentUser.poin > price) {
            const user = AuthService.getCurrentUser();
            if (user) {
                UserService.updateCoin({ poin: price, id_user: user.uid, tipe: 'kurang' })
                    .then(res => {
                        editProfil();
                    }, (error) => {
                        console.log("Private page", error.response);
    ...
    return (
        <>
            <Toaster />
            <div className='bg-custom-primary min-h-screen flex flex-col items-center py-4 md:py-8'>
                <div className='w-90% flex items-center justify-between'>
                    <button onClick={() => navigate('/home')}>
                        <ChevronLeftIcon className='w-10 h-10 text-white bg-custom-secondary rounded-full' />
                    </button>

                    {/* <h1 className='text-white font-custom-font font-medium text-2xl'>Toko Penukaran</h1> */}
                ...
                {/* <div className='w-90% flex bg-custom-yellow shadow-custom-shadow-yellow mb-8 p-4 rounded-lg'>
                    <div className='w-3/4 flex flex-col justify-evenly'>
                        ...
                        <div className='w-3/4 bg-white p-1 rounded-full pl-4'>
                            <p>Coin ku : <strong>{currentUser && currentUser.poin}</strong></p>
                        </div>
                    </div>
                ...
                <div className='w-90% flex flex-wrap'>
                    {dataToko.map((e, i) => (<button
                        key={i}
                        className={`w-[30%] mx-[1.5%] mb-8 flex flex-col justify-between items-center ${listFotoProfil.some(f => f.nama === e?.nama_barang && f.unlock === true) ? 'bg-gray-300 shadow-custom-shadow-gray' :  'bg-custom-yellow shadow-custom-shadow-yellow'}  rounded-md`}
                    ...
                    >
                        ...
                        <img className='my-4' src={`/assets/${e?.nama_barang}.svg`} alt="profil" />
                        <div className='bg-custom-text text-sm text-white py-1 px-2 -mb-2 shadow-custom-shadow-gray rounded-full'>
                            {listFotoProfil.some(f => f.nama === e?.nama_barang && f.unlock === true) ? 'Sudah dimiliki' : `${e?.harga} coin`}
                        </div>
                    </button>))}
